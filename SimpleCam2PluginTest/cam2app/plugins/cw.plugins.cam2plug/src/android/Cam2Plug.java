...
/*Context for sending events to js:*/
 ...
 HandlerThread j2JSMQ_Thread                  = null;
 Handler       j2JSMQ_ThreadHandler           = null;
 Message       msgCtx                         = new Message();
 Message       msgJ2JS                        = new Message();
...
/*======Exec method together with a wrapper to deal with potential problems with plugins delayed initialisation:======================================================*/
 ...
 private boolean checkInitStateWrapper(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
  ...
  else if (action.equals("j2jsLinkCreate"))       {this.commsCallbackContext = callbackContext; msgCtx.obj = this.commsCallbackContext; msgCtx.what = 1; j2JSMQ_ThreadHandler.sendMessage(msgCtx); return true;}
  /*-------------And actions, that do:-----------------------------------------------------*/
...
/*======Initialisation, split in two parts with later one being deferred until proper hardware permissions are obtained:==============================================*/
 ...
 private class J2JSMessageQueue extends HandlerThread {
   private String          lTGbase         = "[j2JSMessageQueue"; /*lTG=lTGbase+".<methods' name>] ";*/
   private CallbackContext callbackContext = null;
   private JSONObject      queue           = new JSONObject();
   public  Handler         localHandler;

   public                      J2JSMessageQueue     ()                                     {super("J2JSMessageQueue");}
   @Override protected void    onLooperPrepared     ()                                     {this.localHandler = new Handler(getLooper()) {@Override public void handleMessage(Message msg) {switch(msg.what) {case 1: setCallbackContext((CallbackContext)msg.obj);break; case 2: enqueueMessage((JSONObject)msg.obj);break; default: break;}}};}
   public              void    setCallbackContext   (CallbackContext commsCallbackContext) {callbackContext=commsCallbackContext;workTheQueue();}
   public              boolean isCallbackContextSet ()                                     {return ((callbackContext==null)||(callbackContext.isFinished()))?false:true;}
   public              void    enqueueMessage       (JSONObject msg)                       {try {queue.putOpt(String.valueOf(queue.length()),msg);} catch(JSONException e) {LOG.e(gTG, lTGbase + ".enqueueMessage(...)] " + "JSONException!" + e.getMessage());} workTheQueue();}
   private             void    workTheQueue         ()                                     {
     if (!isCallbackContextSet()) {LOG.d(gTG, lTGbase + ".workTheQueue] " + "Have messages in the send queue, but can't send because callbackContext is either null (i.e. js hasn't yet called us to establish comms link) or callbackContext.isFinished() returns true (i.e. it has already been used at least once and PluginResult.KeepCallback wasn't set to true at the moment, so callbackContext burned after one use)...");return;}
     for(int i=0;i<queue.length();i++) {JSONObject msg=queue.optJSONObject(String.valueOf(i));if (msg==null) {continue;}PluginResult dataResult=new PluginResult(PluginResult.Status.OK,msg);dataResult.setKeepCallback(true);callbackContext.sendPluginResult(dataResult);queue.remove(String.valueOf(i));}
   }
 }
 ...
 @Override public void pluginInitialize() {
  ...
  LOG.d(gTG, lTG+"Instantiating J2JSMessageQueue and launching it in another thread...");
  try {
    if (j2JSMQ_Thread==null)        {j2JSMQ_Thread  = new J2JSMessageQueue(); j2JSMQ_Thread.start();}
    if (j2JSMQ_ThreadHandler==null) {j2JSMQ_ThreadHandler = new Handler(j2JSMQ_Thread.getLooper());}
    if (commsCallbackContext!=null) {if (!commsCallbackContext.isFinished()) {msgCtx.obj = this.commsCallbackContext; msgCtx.what = 1; j2JSMQ_ThreadHandler.sendMessage(msgCtx);}}
  }
  catch(IllegalThreadStateException e) { LOG.e(gTG, lTG+"Exeption in launching the J2JSMessageQueue thread! "+e.getMessage()); }
  catch(Exception e)                   { LOG.e(gTG, lTG+"Exeption in launching the J2JSMessageQueue thread! "+e.getMessage()); }
  LOG.d(gTG, lTG+"Done instantiating J2JSMessageQueue and launching it in another thread.");
...
/*===============================Assorted utilities===================================================================================================================*/
 private void sendToJs(JSONObject msg) { /*Send messages to J2JSMessageQueue to be enqueued and sent*/
   msgJ2JS.obj  = msg;
   msgJ2JS.what = 2;
   j2JSMQ_ThreadHandler.sendMessage(msgJ2JS);
...
/*~~~notifyJs_xxx(...) functions:~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
 private void notifyJs_bool(String propName, boolean propValue, JSONObject... flagsObjectsArray) {
   ...
   sendToJs(tmpObj); 
   LOG.d(gTG, lTG+"Notification enqueued/sent....");
 }
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
 ...
 @Override public void onPause(boolean multitasking) {/*App loses focus*/
  ...
  if (j2JSMQ_ThreadHandler != null) {j2JSMQ_ThreadHandler.removeCallbacksAndMessages(null);j2JSMQ_ThreadHandler=null;}
  if (j2JSMQ_Thread != null)        {j2JSMQ_Thread.quitSafely();try{j2JSMQ_Thread.join();}catch(InterruptedException e){LOG.e(gTG,lTG+"InterruptedException! "+e.getMessage());}}
...
//       camDataContainer.put("streamConfigurationMap",streamConfigurationMap.toString()); /*...put(Sting,String) for the time being, until I figurew out objects' structure to convert it to JSONObject */
       ...
       for (int effect : camEffects0) {
         ...
         LOG.d(gTG, lTG+"Effect available: " + effect);
       }
